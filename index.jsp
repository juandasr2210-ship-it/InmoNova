...
<section class="hero-nova text-center mb-4">
  <h1 class="display-5">Encuentra tu <span class="grad">hogar ideal</span> con InmoNova</h1>
  <p class="lead opacity-75">Casas, apartamentos, locales, oficinas y terrenos en Santander y Colombia</p>
  <form class="search-card row g-2 justify-content-center p-3 mt-4 text-start" action="catalogo.jsp" method="get">
...
<h3 class="sec-title">✨ Publicaciones destacadas</h3>
<div class="row">
<%
...
try{
  ...
  ps=con.prepareStatement("SELECT p.id_propiedad,p.titulo,p.precio,p.direccion,c.nombre AS ciudad,t.nombre AS tipo,i.nombre_empresa,(SELECT url FROM imagen_propiedad WHERE id_propiedad=p.id_propiedad LIMIT 1) AS foto FROM propiedad p INNER JOIN ciudad c ON p.id_ciudad=c.id_ciudad INNER JOIN tipo_propiedad t ON p.id_tipo=t.id_tipo INNER JOIN inmobiliaria i ON p.id_inmobiliaria=i.id_inmobiliaria WHERE p.activo=1 AND p.estado='disponible' ORDER BY p.destacada DESC LIMIT 6");
  rs=ps.executeQuery();
  boolean hay=false;
  ...
  <div class="col-12 col-md-4 mb-4">
    <div class="card prop-card h-100">
      ...
      <div class="card-body">
        ...
        <p class="text-muted mb-1 small"><%= rs.getString("direccion") %> | <%= rs.getString("nombre_empresa") %></p>
...
<div class="row mt-4 g-3">
  <div class="col-md-4"><div class="panel-card h-100"><h5>🔍 Busca y filtra</h5><p class="text-muted mb-0">Por ciudad, tipo, precio y características.</p></div></div>
  <div class="col-md-4"><div class="panel-card h-100"><h5>📅 Agenda visitas</h5><p class="text-muted mb-0">Sin cruces de agenda (UNIQUE propiedad+fecha).</p></div></div>
  <div class="col-md-4"><div class="panel-card h-100"><h5>🔒 Acceso por rol</h5><p class="text-muted mb-0">Cliente, inmobiliaria y administrador con paneles distintos.</p></div></div>
</div>
